...
// completed.
func benchWrap(b *testing.B, options *badgerhold2.Options, bench func(store *badgerhold2.Store, b *testing.B)) {
	...
	if options == nil {
		options = &badgerhold2.DefaultOptions
	}

	options.Dir = tempDir
	options.ValueDir = tempDir

	store, err := badgerhold2.Open(*options)
	if err != nil {
		b.Fatalf("Error opening %s: %s", tempDir, err)
	}
...
func BenchmarkRawInsert(b *testing.B) {
	data, err := badgerhold2.DefaultEncode(benchItem)
	if err != nil {
		b.Fatalf("Error encoding data for raw benchmarking: %s", err)
	}

	benchWrap(b, nil, func(store *badgerhold2.Store, b *testing.B) {
		b.ResetTimer()
...
func BenchmarkNoIndexInsertJSON(b *testing.B) {
	opt := badgerhold2.DefaultOptions
	opt.Encoder = json.Marshal
	opt.Decoder = json.Unmarshal
	benchWrap(b, &opt, func(store *badgerhold2.Store, b *testing.B) {
		b.ResetTimer()
...
func BenchmarkFindNoIndex(b *testing.B) {
	benchWrap(b, nil, func(store *badgerhold2.Store, b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			...
			err := store.Find(&result, badgerhold2.Where("Category").Eq("findCategory"))
			if err != nil {
