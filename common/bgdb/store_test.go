...
var globalStore *badgerhold2.Store

func TestMain(m *testing.M) {
	opt := testOptions()
	var err error
	globalStore, err = badgerhold2.Open(opt)
	if err != nil {
...
func TestOpen(t *testing.T) {
	opt := testOptions()
	store, err := badgerhold2.Open(opt)
	if err != nil {
...
func TestAlternateEncoding(t *testing.T) {
	...
	store.Find(&result, badgerhold2.Where(badgerhold2.Key).Eq(tData.Key))
...
// completed.
func testWrap(t *testing.T, tests func(store *badgerhold2.Store, t *testing.T)) {
	tests(globalStore, t)
...
func testOptions() badgerhold2.Options {
