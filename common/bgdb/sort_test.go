...
var sortTests = []test{
	test{
		name:   "Sort By Name",
		query:  badgerhold2.Where("Category").Eq("animal").SortBy("Name"),
		result: []int{9, 5, 14, 8, 13, 2, 16},
	},
	test{
		name:   "Sort By Name Reversed",
		query:  badgerhold2.Where("Category").Eq("animal").SortBy("Name").Reverse(),
		result: []int{16, 2, 13, 8, 14, 5, 9},
	},
	test{
		name:   "Sort By Multiple Fields",
		query:  badgerhold2.Where("ID").In(8, 3, 13).SortBy("Category", "Name"),
		result: []int{13, 15, 4, 3},
	},
	test{
		name:   "Sort By Multiple Fields Reversed",
		query:  badgerhold2.Where("ID").In(8, 3, 13).SortBy("Category", "Name").Reverse(),
		result: []int{3, 4, 15, 13},
	},
	test{
		name:   "Sort By Duplicate Field Names",
		query:  badgerhold2.Where("ID").In(8, 3, 13).SortBy("Category", "Name", "Category"),
		result: []int{13, 15, 4, 3},
	},
	test{
		name:   "Sort By Name with limit",
		query:  badgerhold2.Where("Category").Eq("animal").SortBy("Name").Limit(3),
		result: []int{9, 5, 14},
	},
	test{
		name:   "Sort By Name with skip",
		query:  badgerhold2.Where("Category").Eq("animal").SortBy("Name").Skip(3),
		result: []int{8, 13, 2, 16},
	},
	test{
		name:   "Sort By Name with skip and limit",
		query:  badgerhold2.Where("Category").Eq("animal").SortBy("Name").Skip(2).Limit(3),
		result: []int{14, 8, 13},
	},
	test{
	...
	test{
		...
		query:  badgerhold2.Where("Category").Eq("animal").SortBy("Name").Skip(4),
		result: []int{13, 2, 16},
	},
	test{
	...
	test{
		...
		query:  badgerhold2.Where("Category").Eq("animal").SortBy("Name").Skip(10),
		result: []int{},
	},
}
	...
	for _, tst := range sortTests {
		t.Run(tst.name, func(t *testing.T) {
			testWrap(t, func(store *badgerhold2.Store, t *testing.T) {
				...
				var result []ItemTest
				err = store.Find(&result, badgerhold2.Where("UpdateIndex").Eq("updated index").And("UpdateField").Eq("updated"))
				if err != nil {
...
func TestSortOnKey(t *testing.T) {
	testWrap(t, func(store *badgerhold2.Store, t *testing.T) {
		...
		var result []ItemTest
		_ = store.Find(&result, badgerhold2.Where("Name").Eq("blah").SortBy(badgerhold2.Key))
	})
}
...
func TestSortedFindOnInvalidFieldName(t *testing.T) {
	testWrap(t, func(store *badgerhold2.Store, t *testing.T) {
		...
		err := store.Find(&result, badgerhold2.Where("BadFieldName").Eq("test").SortBy("BadFieldName"))
		if err == nil {
...
func TestSortedFindWithNonSlicePtr(t *testing.T) {
	testWrap(t, func(store *badgerhold2.Store, t *testing.T) {
		...
		var result []ItemTest
		_ = store.Find(result, badgerhold2.Where("Name").Eq("blah").SortBy("Name"))
	})
}
