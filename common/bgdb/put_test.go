...
import (
	...
	"github.com/pmker/oneplus/common/bgdb"
...
func TestInsert(t *testing.T) {
	testWrap(t, func(store *badgerhold2.Store, t *testing.T) {
		...
		if err != badgerhold2.ErrKeyExists {
			t.Fatalf("Insert didn't fail! Expected %s got %s", badgerhold2.ErrKeyExists, err)
		}
...
func TestUpdate(t *testing.T) {
	testWrap(t, func(store *badgerhold2.Store, t *testing.T) {
		...
		if err != badgerhold2.ErrNotFound {
			t.Fatalf("Update without insert didn't fail! Expected %s got %s", badgerhold2.ErrNotFound, err)
		}
...
func TestUpsertReadTxn(t *testing.T) {
	testWrap(t, func(store *bgdb.Store, t *testing.T) {
		key := "testKey"
		data := &ItemTest{
			Name:     "Test Name",
			Category: "Test Category",
	...
	for _, tst := range testResults {
		t.Run(tst.name, func(t *testing.T) {
			testWrap(t, func(store *badgerhold2.Store, t *testing.T) {
				...
				var result []ItemTest
				err = store.Find(&result, badgerhold2.Where("UpdateIndex").Eq("updated index").
...
func TestIssue14(t *testing.T) {
	testWrap(t, func(store *badgerhold2.Store, t *testing.T) {
		...
		err = store.Find(&result, badgerhold2.Where("Category").Eq("Test Category"))
		if err != nil {
...
func TestIssue14UpdateMatching(t *testing.T) {
	testWrap(t, func(store *badgerhold2.Store, t *testing.T) {
		...
		err = store.UpdateMatching(&ItemTest{}, badgerhold2.Where("Name").Eq("Test Name"),
...
func TestInsertSequence(t *testing.T) {
	testWrap(t, func(store *badgerhold2.Store, t *testing.T) {
		...
		for i := 0; i < 10; i++ {
			err := store.Insert(badgerhold2.NextSequence(), &SequenceTest{})
			if err != nil {
...
func TestInsertSequenceSetKey(t *testing.T) {
	testWrap(t, func(store *badgerhold2.Store, t *testing.T) {
		...
		for i := 0; i < 10; i++ {
			...
			err := store.Insert(badgerhold2.NextSequence(), &st)
			if err != nil {
				t.Fatalf("Error inserting data for sequence test: %s", err)
			}
			if int(st.Key) != i {
...
func TestAlternateTags(t *testing.T) {
	testWrap(t, func(store *badgerhold2.Store, t *testing.T) {
		...
		err = store.Find(&result, badgerhold2.Where("Name").Eq(item.Name).Index("Name"))
		if err != nil {
...
func TestUniqueConstraint(t *testing.T) {
	testWrap(t, func(store *badgerhold2.Store, t *testing.T) {
		...
		err := store.Insert(badgerhold2.NextSequence(), item)
		if err != nil {
		...
		t.Run("Insert", func(t *testing.T) {
			err = store.Insert(badgerhold2.NextSequence(), item)
			if err != badgerhold2.ErrUniqueExists {
				t.Fatalf("Inserting duplicate record did not result in a unique constraint error: "+
					"Expected %s, Got %s", badgerhold2.ErrUniqueExists, err)
			}
		})
		...
		t.Run("Update", func(t *testing.T) {
			...
			err = store.Insert(badgerhold2.NextSequence(), update)
		...
		t.Run("UpdateMatching", func(t *testing.T) {
			...
			err = store.UpdateMatching(TestUnique{}, badgerhold2.Where(badgerhold2.Key).Eq(update.Key),
