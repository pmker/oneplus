...
import (
	"fmt"
	"github.com/pmker/oneplus/common/kvstore/badgerdb"
 	"reflect"
	"strings"
	"sync"
...
func Open(options Options) (*Store, error) {
	...
	cache,err:=badgerdb.SetBadger(db)
	if err!=nil{
		fmt.Printf("cache error %s",err.Error())
	}
	return &Store{
		db:               db,
...
func (s *Store) Cache() *badgerdb.BadgerDB {
	return s.cache
}
