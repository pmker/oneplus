...
func TestGetMissedEventsToBackfillSomeMissed(t *testing.T) {
	// Fixture will return gateway 30 as the tip of the chain
	...
	require.NoError(t, err)
	defer meshDB.Close()
	// Add gateway number 5 as the last gateway seen by BlockWatcher
	...
	// Check that gateway 30 is now in the DB, and gateway 5 was removed.
...
func TestGetMissedEventsToBackfillNoneMissed(t *testing.T) {
	// Fixture will return gateway 5 as the tip of the chain
	...
	// Check that gateway 5 is still in the DB
	headers, err := meshDB.FindAllMiniHeadersSortedByNumber()
	require.NoError(t, err)
	require.Len(t, headers, 1)
